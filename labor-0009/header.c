...
{
    t = clock() - t;
    fprintf(stderr, "{ Signal <Ctrl> + <C>... }\n");
    printf("  Execution time = %f\n", (double)t / CLOCKS_PER_SEC);
    fprintf(stderr, "{ Skibo Dmytro Viacheslavovich 335a }\n");
    kill(getpid(), SIGABRT);
}
...
int find_signature(int f, char* sign)
{
    ...
    ssize_t sizetbuf = read(f, buf, size);
    if (memmem(buf, sizetbuf, sign, sizeof(sign)) != NULL)
    {
        free(buf);
        return 1;
    }
